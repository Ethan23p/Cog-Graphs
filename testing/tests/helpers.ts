...
// is also the only way to assert real exit codes (testing/harness/HARNESS-IMPLEMENTATION.md E4).
//
 ...
 *
 * It lives inside the repo, so it is only as fast as the repo's drive. On a spinning hard
 * disk creating a `.sqlite` costs ~700ms (up to ~4.6s under load), against ~10ms on an
 * SSD (engine/IMPLEMENTATION.md EN4, 2026-09-12 — an earlier reading blamed antivirus,
 * and was wrong). That is environment, not defect, which is why `bun test` runs with
 * `--timeout 20000` rather than each slow case being trimmed to fit — the default 5s
 * made DE-7 and DE-19.4 fail about half of all runs on the hard disk, and an
 * intermittently-lying suite is worse than a red one. Prefer `makeSandbox` unless a case
 * specifically needs a directory outside the temp root.
 */
...
/** LF-normalize for content comparison (see testing/harness/HARNESS-IMPLEMENTATION.md G1). */
