...
| **U2** | **E1–E5 have not been re-run on 0.3.251.** DKB observed them on 0.3.214; `^0.3.214` resolved to 0.3.251 at install, so 37 patch versions of behavioral drift are unverified. Static shapes were re-checked (see "What changed in the port"); behavior was not. | `bun run eval:smoke && bun run verify:claims` — one live smoke run (~$0.02, ~40s) settles E1–E5 on the current SDK. Repeat after any bump. E1–E4 are precisely the claims a version bump could silently break, which is why they are executable rather than prose. **This is the first thing to run once `CLAUDE_CODE_OAUTH_TOKEN` is in `.env`.** |
