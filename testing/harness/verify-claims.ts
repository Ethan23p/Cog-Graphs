// Offline re-verification of the SDK behavioral claims in testing/harness/HARNESS-IMPLEMENTATION.md (E1–E4).
//
...
function checkE3(msgs: AnyMsg[]): ClaimResult {
  ...
  if (hits > 0) {
    return {
      ...
      claim,
      verdict: "FAIL",
      detail: `${hits} session_state_changed message(s) present — the documented fallback turn-over signal is now live; revisit testing/harness/HARNESS-IMPLEMENTATION.md E1/E3`,
    };
  }
...
console.log(`\n=== testing/harness/HARNESS-IMPLEMENTATION.md claim verification ===`);
...
console.log(
  "\nE5 (auth) is verified by `bun run eval:smoke` completing at all.\nU1 (plugins/skills under settingSources: []) needs the Cog-Graphs plugin — see testing/harness/HARNESS-IMPLEMENTATION.md.",
);
