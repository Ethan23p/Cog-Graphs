...
worked (including the ones it minted), the disputes Ethan resolved, the decisions the
engine embodies, and how the rubric layer came to be worded.
...
| `engine/IMPLEMENTATION.md` (earlier `engine/DESIGN.md`) | [Engine implementation decisions](#engine-implementation-decisions) |
...
[Engine implementation decisions](#engine-implementation-decisions) and `testing/harness/HARNESS-IMPLEMENTATION.md` record the decisions the
...
*Formerly `engine/IMPLEMENTATION.md`.*
...
| [Cases](#cases) | Every case there is, in slice order, plus minted ones. Navigation. |
| [Disputes](#disputes) | A landed case that looks *wrong*. Ethan resolves. |
| `testing/harness/HARNESS-IMPLEMENTATION.md` | The eval runtime and its SDK claims. |
| **this part** | Engine decisions, the reasoning that produced them, and the failure patterns that keep recurring. |
...
*Cited:* DE-19.3 (amended 2026-09-09, see [Disputes](#disputes)), DE-19.6.1, DE-21,
...
should not have to. See [Disputes](#disputes).
...
- **The IN-9/10/11 amendment: confirmed.** See [Disputes](#disputes).
- **Minted-case numbering: by subject first, by the slice in progress second.** The rule
  is in [The build loop](#the-build-loop). The DE-19.1–19.8 block keeps its provenance numbers, because its IDs
